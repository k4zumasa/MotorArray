...
int t_act[]={400,100,50,20,10};
int t_rest[]={400,100,50,20,10};
...
void setup() {
  ...
  timer.attach_ms(1, timerCallback); // 1ミリ秒ごとにtimerCallback()を呼び出す
}

void timerCallback() {
  // 1ミリ秒ごとに呼び出されるコールバック関数
  // タイミングに関連する処理をここに記述
  if (flag == 0) {
    ResetPins(0);
    flag = -1;
  } else if (flag >= 1 && flag <= 9) {
    // ディレイに代わるタイミングに関連する処理をここに記述
    if (currentMode == 1) {
      for (int j = 0; j < 8; j++) {
        ActuatePin(0, j);
        ActuatePin(1, j);
        ActuatePin(2, j);
        ActuatePin(3, j);
        // delay(t_act[flag-1]); // delay()を削除
        ResetPins(0);
        // delay(t_rest[flag-1]); // delay()を削除
      }
    } else {
      for (int j = 0; j < 8; j++) {
        ActuatePinAlternative(0, j, 2);
        ResetPins(0);
        // delay(t_rest[flag-1]); // delay()を削除
      }
    }
...
void ActuatePinAlternative(int b,int p,int pulsesInSingleDrive){//pulsesInSingleDrive:正転逆転を繰り返す回数
  for(int i=0;i<=pulsesInSingleDrive/2;i++){
    ActuateRow(p);
    delay(t_act[flag-1]/pulsesInSingleDrive/2);
    ActuateRowReverse(p);
    delay(t_act[flag-1]/pulsesInSingleDrive/2);
  }
}
