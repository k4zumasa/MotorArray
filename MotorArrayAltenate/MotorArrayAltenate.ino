...
void loop() {
  if(flag==9){
    ...
    for(int i = 0; i <= 3; i++){
      ActuateRow(i);
      ets_delay_us(t_act[1]);
      ResetPins(0);
    }
  }
  else if(flag == 2){
    for(int i = 0; i <= 3; i++){
      ActuateRowAlternative(i,2);
      // ets_delay_us(t_act[1]);
      ResetPins(0);
    }
  }
...
void ActuateRowAlternative(int p,int pulsesInSingleDrive){//pulsesInSingleDrive:正転逆転を繰り返す回数
  for(int i=0;i<=pulsesInSingleDrive/2;i++){
    ActuateRow(p);
    ets_delay_us(t_act[1]/pulsesInSingleDrive/2);
    ActuateRowReverse(p);
    ets_delay_us(t_act[1]/pulsesInSingleDrive/2);
  }
}
